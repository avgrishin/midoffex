...
uses
  ...
  OleServer, MSXML2_TLB, Placemnt, MemDS, ADODB;

type
  TfParamADO1 = class(TForm)
    Panel1: TPanel;
		Panel2: TPanel;
    Panel3: TPanel;
    Button1: TButton;
    ...
    qParamValue: TADOQuery;
    spParamValueUpd: TADOStoredProc;
    qParams: TADOQuery;
    qDescr: TADOQuery;
	...
	public
		function FormInit(fibparams: TParameters; params: TStringList; ID, Type1: Integer; ADopParamStr: string): Boolean;
		{ Public declarations }
	end;
var
	fParamADO1: TfParamADO1;

implementation

uses DBLookupEh, DBGridEh, ToolCtrlsEh, DM, DBCtrlsEh,
  StrUtils, Menus, SpravADO1;
...
function TfParamADO1.FormInit(fibparams: TParameters; params: TStringList; ID, Type1: Integer; ADopParamStr: string): Boolean;
var
	i: integer;
	h: integer;
	w: integer;
...
begin
  ...
  FParams := params;
	...
	w2 := 0;
	FormInit := True;
	qParamValue.Parameters.ParamByName('ID').Value := ID;
	qParamValue.Parameters.ParamByName('ParamType').Value := Type1;
	spParamValueUpd.Parameters.ParamByName('@ID').Value := ID;
	spParamValueUpd.Parameters.ParamByName('@ParamType').Value := Type1;
	for i := 0 to FParams.Count-1 do
	begin
    FParams.Strings[i] := StringReplace(FParams.Strings[i], ':', '', [rfReplaceAll]);
		qParamValue.Parameters.ParamByName('Name').Value := FParams.Strings[i];
		qParamValue.Close;
		qParamValue.Open;
		qParams.Parameters.ParamByName('Name').Value := UpperCase(FParams.Strings[i]);
		qParams.Close;
		qParams.Open;
		if qParams.Eof then
		begin
            ...
            begin
              ...
              begin
                ...
                try
                  ...
                  begin
                    if q.Parameters.FindParam(dp.Names[P]) <> nil then
                      q.Parameters.ParamByName(dp.Names[P]).Value := dp.Values[dp.Names[P]];
                  end
                finally
                  FreeAndNil(dp);
                end;
        ...
        6:
            ...
            begin
              ...
              try
                ...
                for P := 0 to dp.Count-1 do
                begin
                  if qParamsNAME.Value = dp.Names[P] then
                    q.Parameters[0].Value := dp.Values[dp.Names[P]];
                end
              finally
                FreeAndNil(dp);
              end;
            end;
            if q.Parameters[0].Value = Null then
            begin
  		  			if not qParamValue.Eof
              then q.Parameters[0].Value := qParamValuePARAMVALUE.AsVariant
              else q.Parameters[0].Value := Null;
            end;
		  			q.Open;
			  		ds := TDataSource.Create(Self);
				  	ds.DataSet := q;
...
procedure TfParamADO1.Button1Click(Sender: TObject);
var
	i: integer;
	tc: TControl;
  param: TParameter;
...
begin
	...
	begin
        ...
        begin
					if ((Panel2.Components[i] as TDBEditEh).DataSource.DataSet as TADOQuery).Parameters[0].Value = Null then
						Abort;
        end
    ...
    begin
      if tc is TDateEdit then
		  	...
		  	val := (tc as TDBLookupComboboxEh).Value
	 			...
	 			val := ((tc as TDBEditEh).DataSource.DataSet as TADOQuery).Parameters[0].Value
      ...
      if param <> nil then param.Value := val;
			spParamValueUpd.Parameters.ParamByName('@Name').Value := FParams.Strings[i];
      if val = Null then
        spParamValueUpd.Parameters.ParamByName('@ParamValue').Value := Null
      else
  			spParamValueUpd.Parameters.ParamByName('@ParamValue').Value := String(val);
...
procedure TfParamADO1.lcDocTW1EditButtons0Click(Sender: TObject;
  var Handled: Boolean);
begin
end;


procedure TfParamADO1.Button2Click(Sender: TObject);
begin
  //
end;

procedure TfParamADO1.btnFormClick(Sender: TObject; var Handled: Boolean);
var
  mr: Integer;
  q: TADOQuery;
  OutValue: Variant;
begin
  q := (((Sender as TEditButtonControlEh).Parent as TDBEditEh).DataSource.DataSet as TADOQuery);
  ShowSprav(q.Tag, q.Parameters[0].Value, OutValue, mr);
  if mr = mrOk then
  begin
  	q.Parameters[0].Value := OutValue;
    q.Close;
    q.Open;
  end;
end;
...
procedure TfParamADO1.btnClearClick(Sender: TObject; var Handled: Boolean);
var
  q: TADOQuery;
begin
  q := (((Sender as TEditButtonControlEh).Parent as TDBEditEh).DataSource.DataSet as TADOQuery);
  q.Parameters[0].Value := Null;
  q.Close;
  q.Open;
end;
