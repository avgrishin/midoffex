...
uses
  ...
  MemDS, DBAccess, MSAccess, ADODB;

type
  TfTableADO1 = class(TForm)
    AvEhGrid1: TAvEhGrid;
    ds1: TDataSource;
...
procedure TfTableADO1.acFilter1Execute(Sender: TObject);
begin
	q1.Filtered := False;
	q1.Filter := '';
end;

procedure TfTableADO1.acFilter1Update(Sender: TObject);
begin
	acFilter1.Enabled := q1.Filtered;
	acFilter1.Checked := q1.Filtered;
end;

procedure TfTableADO1.AvEhGrid1DblClick(Sender: TObject);
begin
  if not q1.IsEmpty then
    acExec1Execute(Sender);
end;

procedure TfTableADO1.acExec1Update(Sender: TObject);
begin
  acExec1.Enabled := not q1.IsEmpty;
end;

procedure TfTableADO1.acExec1Execute(Sender: TObject);
begin
  //
end;

procedure TfTableADO1.acRefresh1Execute(Sender: TObject);
begin
...
procedure TfTableADO1.acEscape1Execute(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TfTableADO1.acExcel1Execute(Sender: TObject);
begin
  Screen.Cursor := crSQLWait;
...
procedure TfTableADO1.acCopy1Execute(Sender: TObject);
begin
  if AvEhGrid1.SelectedField <> nil then
    Clipboard.SetTextBuf(PChar(AvEhGrid1.SelectedField.AsString));
end;

procedure TfTableADO1.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;
